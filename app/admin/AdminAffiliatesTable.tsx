...
// ─── Mini chart — barras + linha (estilo profit/loss) ────────────────────────
...
function MiniSalesChart({ data, up }: { data: number[]; up: boolean }) {
  ...
  const barColor = up ? '#bbf7d0' : '#fecaca'

  const points = data.map((v, i) => {
    const barH = Math.max((v / max) * (H - 4), v > 0 ? 3 : 1)
    return { x: i * barW + barW / 2, y: H - barH }
  })
      ...
      {data.map((v, i) => {
        ...
        return (
          <rect key={i}
            ...
            rx={1.5}
            fill={i === data.length - 1 ? color : barColor}
          />
        )
      })}
      <polyline
...
// ─── Seta de tendência — só seta + % sem texto ───────────────────────────────

function TrendArrow({ current, previous }: { current: number; previous: number }) {
  if (previous === 0 && current === 0) return <span className="text-gray-300 text-xs">—</span>
  if (previous === 0) {
    return (
      <svg width="12" height="12" viewBox="0 0 10 10" fill="#16a34a">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="bg-white rounded-2xl border overflow-hidden" style={{ borderColor: 'var(--color-border)' }}>
          <div className="hidden sm:grid text-xs font-semibold text-gray-500 uppercase tracking-wide px-4 py-2.5 border-b"
            style={{
              borderColor: 'var(--color-border)', background: '#fafafa',
              gridTemplateColumns: '2fr 1fr 80px 1fr 1fr 1fr 72px 52px',
            }}>
            <span>Afiliado</span>
            <span>Código</span>
            <span>Desde</span>
          ...
          <div className="divide-y" style={{ borderColor: 'var(--color-border)' }}>
            {paginated.map(aff => <AffiliateRow key={aff.id} affiliate={aff} />)}
          </div>
        </div>
      )}
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
              className="w-8 h-8 rounded-lg flex items-center justify-center text-sm border disabled:opacity-30 hover:bg-gray-50"
...
function AffiliateRow({ affiliate }: { affiliate: Affiliate }) {
  ...
  const counts       = getMonthlyCounts(affiliate)
  const currentMonth = counts[counts.length - 1]
  const prevMonth    = counts[counts.length - 2]
  const up           = currentMonth >= prevMonth
  ...
  return (
    <>
      <div
        ...
        style={{ gridTemplateColumns: '2fr 1fr 80px 1fr 1fr 1fr 72px 52px' }}
        onClick={() => setExpanded(e => !e)}
      >
        <div className="min-w-0">
          <p className="text-sm font-semibold text-gray-800 truncate">{affiliate.profiles?.full_name || '—'}</p>
        ...
        <div className="flex items-center justify-end gap-2">
          <MiniSalesChart data={counts} up={up} />
          <p className="text-sm font-bold text-gray-800 tabular-nums w-6 text-right">
        ...
        <div className="flex items-center justify-center gap-1.5">
          ...
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none"
            ...
            className="group-hover:stroke-gray-500 transition-colors"
      ...
      {expanded && (
        <div className="px-4 pb-4 pt-3 border-t bg-gray-50" style={{ borderColor: 'var(--color-border)' }}>
          ...
          <div className="flex justify-between items-end">
            <div>
              ...
              <div className="flex items-end gap-1.5">
                {getMonthlyCounts(affiliate).map((v, i, arr) => {
                  ...
                  const barUp = arr[arr.length - 1] >= arr[arr.length - 2]
                  ...
                  return (
                    <div key={i} className="flex flex-col items-center gap-1">
                      <div style={{
                        width: 18,
                        height: Math.max((v / max) * 40, v > 0 ? 4 : 2),
                        background: color,
            ...
            <button onClick={e => { e.stopPropagation(); handleToggle() }} disabled={isPending}
